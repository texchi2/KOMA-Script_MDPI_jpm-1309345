...
\\floatbox[{\\capbeside\\thisfloatsetup{capbesideposition={right,center},capbesidewidth=.35\\linewidth,capbesidesep=quad}}]{figure}[\\FBwidth]
%\centering
{\includegraphics[width =\textwidth, height = 0.6\textheight, keepaspectratio]{Figure2.pdf} %MDPI: Please revise the scientific notation format to a × 10b, and picture is not clear.
} 
*** check *** {\caption{\hl{The initial progress of candidate selection from the TCGA} \acrshort{hnscc} cohort.}{The \protect\textit{p}~value of Kaplan--Meier survival was one of the selection criteria.
} 
The effect size was estimated by Cox's hazard ratio.
Initial trial step: (\textbf{a}) univariate HR versus FDR-adjusted \protect\textit{p}~value; (\textbf{b}) multivariate HR versus FDR-adjusted \protect\textit{p}~value.
After stringent restriction by Bonferroni-adjusted \protect\textit{p}~values and Cox's HR, a few top-ranked genes were acquired by
(\textbf{c}) univariate HR versus Bonferroni-adjusted \protect\textit{p}~value;  (\textbf{d}) multivariate HR versus Bonferroni-adjusted \protect\textit{p}~value.
...
*** check *** {\caption{\hl{Kaplan--Meier survival analyses, by cutoff finding.}}
} 
...
*** check *** {\caption{\hl{The concept of holistic care for} \acrshort{hnscc} patients.} %MDPI: please confirm if the extra lines and frame need to be deleted.
} 
