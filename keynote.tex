...
\newcommand*{\mydate}{2021/09/10} %{\today} % Presentation date {2021/09/10}
...
%\cite{Chi2021}
\includegraphics[width=7cm]{TMU_PTM_logo.png}

\large \hspace{5cm} A Holistic Way for Cancer Care
...
\begin{figure}
  ...
  \begin{picture}(15, 20) % zero point at (15,20) %(1,0.55038404)%
\centering
...
\large TCGA's \acrfull{hnscc}: n = 414, for \hl{modeling}\\ %528\\
\indent{GSE65858 cohort~\cite{Wichmann2015}: n = 270, for \hl{validation}}%288}
...
%\noindent{the effect sizes (coefficients, i.e., hazard ratios)}
...
\thispagestyle{headings}
\markright{Results \hfill The way to find CAMK2N1, CALML5, and FCGBP \hfill}
...
% top 3 genes
\thispagestyle{headings}
\markright{Results \hfill FDR then Bonferroni \hfill}
...
%%
\thispagestyle{headings}
\markright{Results\hfill Step 1: FDR-adjustment \hfill}
...
\thispagestyle{headings}
\markright{Results\hfill Step 2: Bonferroni-adjustment \hfill}
...
Clinical tumor size, surgical margin involvement, and CAMK2N1 expression are independent prognostic factors in HNSCC (TCGA) cohort.}
...
%
\thispagestyle{headings}
\markright{Discussion \hfill Heterogeneity of datasets \hfill}
...
%\hfill
    ...
    \caption{Correlations of Cox's hazard ratios of those preliminary candidate genes.}% (Pearson's r = 0.68).}
    \end{subfigure}    
}   
{\captionsetup{labelformat=empty}    
...
\subsection{Limitations of Current HNSCC Therapy}
